...
import {Typography, Card, Row, Col, Pagination, message, Button, Input, Modal, List} from "antd";
...
const MyNFTs: React.FC = () => {
    ...
    try {
      ...
      setOffers(
        offersList.map((offer: { buyer: string; offer_price: string }, index: number) => ({
          offerId: index + 1, // Assign a temporary ID since none is provided
          nftId,
          offerer: offer.buyer,
  ...
  const handleDeclineOffer = async (offer: Offer) => {
    try {
      ...
      setOffers(offers.filter((o) => o.offerer !== offer.offerer));
    } catch (error) {
  ...
  const handleConfirmTip = async () => {
    ...
    try {
      const tipInOctas = Math.floor(parseFloat(tipAmount) * 1e8); // Convert APT to Octas and ensure it's an integer
      if (isNaN(tipInOctas) || tipInOctas <= 0) {
        message.error("Invalid tip amount.");
        return;
      }

      // Ensure NFT ID is passed as a valid u64 (number)
      const nftIdAsString = selectedNftForTip.id.toString();
      // Assuming `selectedNftForTip.id` is a numeric string
      ...
      const entryFunctionPayload = {
        ...
        arguments: [
          ...
          nftIdAsString, // NFT ID as u64
          tipInOctas, // Tip amount in octas
        ],
      };

      console.log("Submitting tip transaction:", entryFunctionPayload);

      // Submit transaction via Aptos wallet
      const response = await (window as any).aptos.signAndSubmitTransaction(entryFunctionPayload);
      await client.waitForTransaction(response.hash);
